...
Main.b:
			...
			pushq %r15
Main_b_1:
			# Start of method Main_b
			# Initialize string, PLZ FOR THE LOVE OF GOD WORK\n
			# Push caller saved registers
			pushq %rbx
			pushq %rcx
			pushq %rdx
...
Main_main_2:
			...
			# Move vtable pointer into rax
			movq 16(%rbx), %rax
			# Move vtable pointer + constructor offset into rax
			movq 8(%rax), %rax
			call *%rax
			# Pop caller saved registers
			popq %r11
			...
			movq 80(%rsp), %rax
			movq %rax, 0(%rsp)
			# Move receiver object into self, call function out_string
			movq %r8, %rbx
			# Move vtable pointer into rax
			movq 16(%r8), %rax
			# Move vtable pointer + offset into rax
...
string_constant..0:
			.string "PLZ FOR THE LOVE OF GOD WORK\n"
