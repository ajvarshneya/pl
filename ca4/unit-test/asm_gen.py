...
def asm_gen(blocks, spilled_registers, cool_type, c_map, i_map):
	...
	for block in blocks:
		for inst in block.insts:
			...
			elif isinstance(inst, TACLessThan):
				asm_less_than(inst, asm)
			elif isinstance(inst, TACLessEquals):
				asm_less_equals(inst, asm)
			elif isinstance(inst, TACEquals):
				asm_equals(inst, asm)
...
def asm_dynamic_dispatch(inst, cool_type, i_map, asm):
	asm += [comment('dynamic_dispatch')] # debugging label
...
def asm_static_dispatch(inst, cool_type, i_map, asm):
	asm += [comment('static_dispatch')] # debugging label
...
def asm_self_dispatch(inst, cool_type, i_map, asm):
	asm += [comment('self_dispatch')] # debugging label
...
def asm_less_than(inst, asm):
	asm += [comment("less_than")]
	assignee = get_color(inst.assignee)
	op1 = get_color(inst.op1)
	op2 = get_color(inst.op2)

	asm += [comment("compare " + op1 + " < " + op2)]
	asm_push_caller(asm)	

	asm += [comment("push operands")]
	asm += [pushq(op2)]
	asm += [pushq(op1)]
...
def asm_less_equals(inst, asm):
	asm += [comment("less_equals")]
	assignee = get_color(inst.assignee)
	op1 = get_color(inst.op1)
	op2 = get_color(inst.op2)
...
def asm_new(inst, asm):
	asm += [comment('new')] # debugging label
...
def asm_jmp(inst, asm):
	asm += [comment('jump')] # debugging label
...
def asm_return(inst, asm):
	asm += [comment('return')] # debugging label
	value = get_color(inst.op1)
	asm += [movq(value, '%rax')]

def asm_box(inst, asm):
	asm += [comment('box')] # debugging label
