...
Main_attr_init:
			...
			# Start of attribute init Main_a
...
Main_main_0:
			...
			# Push parameters on in reverse
			subq $0, %rsp
			# Move receiver object into self, call function type_name
			movq %r8, %rbx
			# Move vtable pointer into rax
			movq 16(%r8), %rax
			# Move vtable pointer + offset into rax
			...
			# Push parameters on in reverse
			subq $0, %rsp
			# Move receiver object into self, call function get_a
			movq %r8, %rbx
			# Move vtable pointer into rax
			movq 16(%r8), %rax
			# Move vtable pointer + offset into rax
...
.globl Main.get_a
Main.get_a:
			# Method definition for Main.get_a
			pushq %rbp
			movq %rsp, %rbp
			# Push callee saved registers
			pushq %r12
			...
			pushq %r15
Main_get_a_1:
			# Start of method Main_get_a
			# Loading attribute a
			movq 24(%rbx), %r8
			movq %r8, %rax
			# End of method Main_get_a
			# Pop callee saved registers
			popq %r15
			popq %r14
			popq %r13
